...
public class AI : MonoBehaviour
{
        ...
        { // la replacer au centre
            if (Random.Range(1, 100) < returnToCenter)
            {
                transform.position = Vector2.MoveTowards(
                    transform.position,
                    new Vector2(transform.position.x, 1), // Centre de l'écran sur l'axe Y
                    latency * Time.deltaTime);
            }
        }
    }
    ...
    void ChangeVal()
    {
        ...
        {
            latency = (Random.Range(1, 10) > 5) ? (stratingLatency + 0.5f) : (startingLatency - 0.5f);
